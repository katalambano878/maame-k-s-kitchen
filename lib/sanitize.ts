...
/**
 * Validate local phone number format.
 * Adjust the rules below to match the country code(s) you support.
 */
export function isValidLocalPhone(phone: string): boolean {
    if (!phone || typeof phone !== 'string') return false;
    const cleaned = phone.replace(/\D/g, '');
    // Default rules — replace these with your own country's expected lengths/prefixes
    return (
