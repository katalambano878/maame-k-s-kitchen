...
export default function MyTicketsPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'open': return 'bg-[#fdf9ec] text-[#C8952A]';
      case 'in_progress': return 'bg-[#fdf9ec] text-[#C8952A]';
      case 'resolved': return 'bg-[#fdf9ec] text-[#C8952A]';
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case 'high': return 'text-[#C8952A]';
      case 'normal': return 'text-[#C8952A]';
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          {filteredTickets.length === 0 ? (
            <div className="bg-white rounded-xl shadow-sm p-12 text-center">
              ...
              <Link
                href="/support/ticket"
                className="inline-block bg-[#111111] hover:bg-[#111111] text-white px-6 py-3 rounded-lg font-semibold transition-colors whitespace-nowrap"
              >
            ...
            <div className="space-y-4">
              {filteredTickets.map((ticket) => (
                <Link
                  ...
                  className="block bg-white rounded-xl shadow-sm p-6 hover:shadow-lg transition-all border-2 border-transparent hover:border-[#C8952A]"
                >
          ...
          <div className="mt-8 grid md:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl shadow-sm p-6">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg mb-4">
                <i className="ri-time-line text-2xl text-[#C8952A]"></i>
              </div>
              <h3 className="font-bold text-gray-900 mb-2">Response Time</h3>
              <p className="text-2xl font-bold text-[#C8952A] mb-1">24 hours</p>
              <p className="text-sm text-gray-600">Average response time</p>
            </div>

            <div className="bg-white rounded-xl shadow-sm p-6">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg mb-4">
                <i className="ri-checkbox-circle-line text-2xl text-[#C8952A]"></i>
              </div>
              <h3 className="font-bold text-gray-900 mb-2">Resolution Rate</h3>
              <p className="text-2xl font-bold text-[#C8952A] mb-1">95%</p>
              <p className="text-sm text-gray-600">First contact resolution</p>
            </div>

            <div className="bg-white rounded-xl shadow-sm p-6">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg mb-4">
                <i className="ri-customer-service-line text-2xl text-[#C8952A]"></i>
              </div>
              <h3 className="font-bold text-gray-900 mb-2">Support Hours</h3>
              <p className="text-2xl font-bold text-[#C8952A] mb-1">24/7</p>
