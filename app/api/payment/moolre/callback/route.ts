...
export async function POST(req: Request) {
    ...
    try {
        ...
        // ============================================================
        // SECURITY: Verify callback secret only when Moolre actually
        // sends one. Some Moolre accounts don't return a `secret` in
        // the callback body — rejecting those would drop every payment.
        // ============================================================
        ...
        if (expectedSecret && !body.secret) {
            console.warn('[Callback] MOOLRE_CALLBACK_SECRET configured but Moolre did not send `secret` in body. Proceeding (configure the secret in Moolre dashboard to enforce verification).');
        }
        ...
        // ============================================================
        // SECURITY: Robust success/failure detection
        // Treat as success when API status AND tx status both indicate
        // success, OR the message explicitly says "successful/completed/paid",
