...
export default function AdminReviewsPage() {
  ...
  const statusColors: any = {
    ...
    'Approved': 'bg-[#fdf9ec] text-[#C8952A]',
    'Rejected': 'bg-[#fdf9ec] text-[#C8952A]'
  };
    ...
    return (
      <div className="flex items-center space-x-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <i
            key={star}
            className={`${star <= rating ? 'ri-star-fill text-[#C8952A]' : 'ri-star-line text-gray-300'} text-lg`}
          ></i>
        ))}
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button
          ...
          className={`p-4 rounded-xl border-2 transition-all text-left ${statusFilter === 'all' ? 'border-[#C8952A] bg-[#fdf9ec]' : 'border-gray-200 bg-white'
            }`}
        >
        ...
        <button
          ...
          className={`p-4 rounded-xl border-2 transition-all text-left ${statusFilter === 'pending' ? 'border-[#C8952A] bg-[#fdf9ec]' : 'border-gray-200 bg-white'
            }`}
        >
          <p className="text-2xl font-bold text-[#C8952A]">{stats.pending}</p>
          <p className="text-sm text-gray-600 mt-1">Pending Review</p>
        </button>
        <button
          onClick={() => setStatusFilter('approved')}
          className={`p-4 rounded-xl border-2 transition-all text-left ${statusFilter === 'approved' ? 'border-[#C8952A] bg-[#fdf9ec]' : 'border-gray-200 bg-white'
            }`}
        >
          <p className="text-2xl font-bold text-[#C8952A]">{stats.approved}</p>
          <p className="text-sm text-gray-600 mt-1">Approved</p>
        </button>
        <button
          onClick={() => setStatusFilter('rejected')}
          className={`p-4 rounded-xl border-2 transition-all text-left ${statusFilter === 'rejected' ? 'border-[#C8952A] bg-[#fdf9ec]' : 'border-gray-200 bg-white'
            }`}
        >
          <p className="text-2xl font-bold text-[#C8952A]">{stats.rejected}</p>
              ...
              {loading ? (
                ...
                filteredReviews.map((review) => (
                  <tr key={review.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 flex items-center justify-center bg-[#fdf9ec] text-[#C8952A] rounded-full text-xs font-semibold">
