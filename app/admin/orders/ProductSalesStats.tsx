                    ...
                    {(['24h', '7d', '30d', 'all'] as const).map((p) => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${period === p
                                    ? 'bg-[#111111] text-white shadow-md shadow-[#C8952A]'
                            ...
                            {loading ? (
                                <tr>
                                    <td colSpan={4} className="p-12 text-center text-gray-500">
                                        <i className="ri-loader-4-line text-3xl animate-spin text-[#C8952A] mb-2 block"></i>
                                        Loading sales data...
                                    </td>
                                </tr>
                            ) : stats.length === 0 ? (
                                ...
                                stats.map((s) => (
                                    ...
                                    <>
                                    <tr 
                                        className={`hover:bg-[#fdf9ec]/30 transition-colors ${hasVariants ? 'cursor-pointer' : ''}`}
                                    ...
                                    >
                                        <td className="p-4 pl-6 font-medium text-gray-900">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                {hasVariants && (
                                                    <span className="text-[10px] bg-[#fdf9ec] text-[#C8952A] px-1.5 py-0.5 rounded-full font-semibold">
                                        ...
                                        <td className="p-4 text-center">
                                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#fdf9ec] text-[#a07020]">
                                                {s.ordersCount}
                                            </span>
                                        </td>
                                        <td className="p-4 text-center">
                                            <span className="font-bold text-[#C8952A] text-base">{s.itemsSold}</span>
                                        </td>
                                    ...
                                    {expandedProduct === s.productId && hasVariants && (
                                        s.variants.map((v) => (
                                            <tr key={`${s.productId}-${v.name}`} className="bg-gray-50/80 border-l-2 border-[#e8c87a]">
                                                <td className="p-3 pl-14 text-gray-600 text-xs">
                                                    <span className="inline-flex items-center space-x-1.5">
                                                        <i className="ri-price-tag-3-line text-[#C8952A]"></i>
