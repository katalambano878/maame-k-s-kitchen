...
export default function ProductsPage() {
  ...
  const statusColors: any = {
    'active': 'bg-[#fdf9ec] text-[#C8952A]',
    'draft': 'bg-gray-100 text-gray-700',
    'archived': 'bg-[#fdf9ec] text-[#C8952A]',
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
          ...
          <p className="text-2xl font-bold text-[#C8952A]">{stats.active}</p>
        </div>
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
          <p className="text-sm text-gray-600 mb-1">Low Stock</p>
          <p className="text-2xl font-bold text-[#C8952A]">{stats.lowStock}</p>
        </div>
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
          <p className="text-sm text-gray-600 mb-1">Out of Stock</p>
          <p className="text-2xl font-bold text-[#C8952A]">{stats.outOfStock}</p>
        </div>
      </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <div className="flex items-center space-x-3">
              <button
                ...
                className="px-4 py-3 border-2 border-[#C8952A]/50 text-[#C8952A] rounded-lg hover:border-[#C8952A] hover:bg-[#fdf9ec] transition-colors font-medium whitespace-nowrap cursor-pointer"
              >
              ...
              <select
                ...
                className="px-4 py-3 pr-8 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#C8952A] focus:border-[#C8952A] font-medium cursor-pointer"
              >
              ...
              <div className="flex border-2 border-gray-300 rounded-lg overflow-hidden">
                <button
                  ...
                  className={`w-10 h-10 flex items-center justify-center transition-colors cursor-pointer ${viewMode === 'list' ? 'bg-[#111111] text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                    }`}
                >
                ...
                <button
                  ...
                  className={`w-10 h-10 flex items-center justify-center border-l-2 border-gray-300 transition-colors cursor-pointer ${viewMode === 'grid' ? 'bg-[#111111] text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                    }`}
                >
          ...
          <div className="p-4 bg-[#fdf9ec] border-b border-[#e8c87a] flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className="px-4 py-2 bg-[#111111] hover:bg-[#111111] text-white rounded-lg text-sm font-medium transition-colors whitespace-nowrap cursor-pointer"
              >
                Delete
              </button>
            </div>
                  ...
                  <tr key={product.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-4 text-gray-700">
                      ...
                      {product.stock <= (product.metadata?.low_stock_threshold || 5) && product.stock > 0 && (
                        <span className="ml-2 w-2 h-2 rounded-full bg-[#C8952A] inline-block" title="Low Stock"></span>
                      )}
                      {product.stock === 0 && (
                        <span className="ml-2 w-2 h-2 rounded-full bg-[#C8952A] inline-block" title="Out of Stock"></span>
                      )}
                    </td>
                    <td className="py-4 px-4">
            ...
            {filteredProducts.map((product) => (
              <div key={product.id} className="border-2 border-gray-200 rounded-xl p-4 hover:shadow-lg transition-shadow">
                ...
                <div className="flex items-center space-x-2">
                  <Link
                    href={`/admin/products/${product.id}`}
                    className="flex-1 bg-[#111111] hover:bg-[#111111] text-white py-2 rounded-lg text-sm font-medium text-center transition-colors whitespace-nowrap cursor-pointer"
                  >
                    Edit
                  </Link>
                  <button
                    onClick={() => handleDeleteProduct(product.id)}
                    className="w-9 h-9 flex items-center justify-center border-2 border-gray-300 text-gray-700 hover:border-[#C8952A] hover:text-[#C8952A] rounded-lg transition-colors cursor-pointer"
                  >
