...
export default function AdminBlogPage() {
  ...
  const statusColors: any = {
    'Published': 'bg-[#fdf9ec] text-[#C8952A]',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/blog/new" className="bg-[#111111] hover:bg-[#111111] text-white px-6 py-3 rounded-lg font-semibold transition-colors whitespace-nowrap">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
          ...
          <p className="text-2xl font-bold text-[#C8952A]">{posts.filter(p => p.status === 'Published').length}</p>
        </div>
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
          ...
          <p className="text-2xl font-bold text-[#C8952A]">{posts.reduce((sum, p) => sum + p.comments, 0)}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <select className="px-4 py-2 pr-8 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#C8952A] focus:border-[#C8952A] font-medium cursor-pointer">
            ...
            <div className="flex border-2 border-gray-300 rounded-lg overflow-hidden">
              <button
                ...
                className={`w-10 h-10 flex items-center justify-center transition-colors ${
                  viewMode === 'grid' ? 'bg-[#111111] text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                }`}
              >
              ...
              <button
                ...
                className={`w-10 h-10 flex items-center justify-center border-l-2 border-gray-300 transition-colors ${
                  viewMode === 'list' ? 'bg-[#111111] text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                }`}
              >
        ...
        {selectedPosts.length > 0 && (
          <div className="p-4 bg-[#fdf9ec] border-b border-[#e8c87a] flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <button className="px-4 py-2 bg-[#111111] hover:bg-[#111111] text-white rounded-lg text-sm font-medium transition-colors whitespace-nowrap">
                Publish
              </button>
              ...
              <div key={post.id} className="border-2 border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative">
                  <input
                    ...
                    className="absolute top-3 left-3 w-5 h-5 accent-emerald-600 border-gray-300 rounded focus:ring-[#C8952A] cursor-pointer z-10"
                  />
                  <div className="aspect-video bg-gray-100 overflow-hidden">
                    <img src={post.image} alt={post.title} className="w-full h-full object-cover" />
                  </div>
                  {post.featured && (
                    <span className="absolute top-3 right-3 bg-[#C8952A] text-white px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap">
                      Featured
                    </span>
                  )}
                </div>
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-semibold text-[#C8952A]">{post.category}</span>
                  ...
                  <div className="flex items-center space-x-2">
                    <Link
                      ...
                      className="flex-1 bg-[#111111] hover:bg-[#111111] text-white py-2 rounded-lg text-sm font-medium text-center transition-colors whitespace-nowrap"
                    >
                      Edit Post
                    </Link>
                    <button className="w-9 h-9 flex items-center justify-center border-2 border-gray-300 text-gray-700 hover:border-[#C8952A] hover:text-[#C8952A] rounded-lg transition-colors">
                  ...
                  <tr key={post.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <Link href={`/admin/blog/${post.id}`} className="font-semibold text-gray-900 hover:text-[#C8952A]">
                            {post.title}
                          </Link>
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-2">
                        <Link
                          href={`/admin/blog/${post.id}`}
                          className="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-[#C8952A] hover:bg-[#fdf9ec] rounded-lg transition-colors"
                        >
                          <i className="ri-edit-line text-lg"></i>
                        </Link>
                        <button className="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-[#C8952A] hover:bg-[#fdf9ec] rounded-lg transition-colors">
        ...
        <div className="p-6 border-t border-gray-200 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button className="w-10 h-10 flex items-center justify-center bg-[#111111] text-white rounded-lg font-semibold">1</button>
            <button className="w-10 h-10 flex items-center justify-center border-2 border-gray-300 rounded-lg hover:border-gray-400 transition-colors">
