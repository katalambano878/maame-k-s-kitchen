...
export default function CustomerDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (!customer) return <div className="p-8 text-center text-[#C8952A]">Customer not found</div>;
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center space-x-4">
                    ...
                    <div className="w-16 h-16 bg-[#fdf9ec] rounded-full flex items-center justify-center text-[#C8952A] text-2xl font-bold">
                ...
                <div className="flex space-x-3">
                    ...
                    <button className="px-4 py-2 bg-[#111111] text-white rounded-lg font-bold hover:bg-[#111111] cursor-pointer">
                        Edit Customer
                    </button>
                </div>
            </div>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {orders.map(order => (
                                <tr key={order.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 text-sm font-medium text-[#C8952A]">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize
                                            ${order.status === 'completed' || order.status === 'delivered' ? 'bg-[#fdf9ec] text-[#a07020]' :
                                                order.status === 'pending' ? 'bg-[#fdf9ec] text-[#a07020]' :
                                                    order.status === 'cancelled' ? 'bg-[#fdf9ec] text-[#a07020]' : 'bg-[#fdf9ec] text-[#a07020]'}`}>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <Link href={`/admin/orders/${order.id}`} className="text-gray-400 hover:text-[#C8952A]">
