...
export default function AnalyticsPage() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      while (d <= today) {
        const dateKey = d.toLocaleDateString('en-GB', { month: 'short', day: 'numeric' });
        salesMap[dateKey] = {
          date: dateKey,
          sales: 0,
          orders: 0,
      ...
      orders?.forEach(o => {
        const dateKey = new Date(o.created_at).toLocaleDateString('en-GB', { month: 'short', day: 'numeric' });
        if (salesMap[dateKey]) {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <select
              ...
              className="px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#C8952A] focus:border-[#C8952A] font-medium pr-8 cursor-pointer bg-white"
            >
            ...
            <button className="bg-[#111111] hover:bg-[#111111] text-white px-6 py-3 rounded-lg font-semibold transition-colors whitespace-nowrap cursor-pointer flex items-center justify-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[#C8952A] font-semibold text-sm">Live</span>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg">
                <i className="ri-shopping-cart-line text-2xl text-[#C8952A]"></i>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg">
                <i className="ri-bar-chart-box-line text-2xl text-[#C8952A]"></i>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg">
                <i className="ri-percent-line text-2xl text-[#C8952A]"></i>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {topProducts.map((product, index) => (
                    <tr key={index}>
                      ...
                      <td className="py-3 text-right text-sm font-semibold text-[#C8952A]">${product.revenue.toLocaleString()}</td>
                    </tr>
                  ))}
