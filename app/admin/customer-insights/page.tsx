...
export default function CustomerInsightsPage() {
  ...
  const getSegmentBadge = (segment: string) => {
    const badges: any = {
      ...
      returning: 'bg-[#fdf9ec] text-[#C8952A]',
      new: 'bg-[#fdf9ec] text-[#C8952A]',
      'at-risk': 'bg-[#fdf9ec] text-[#C8952A]'
    };
  ...
  const getRiskBadge = (risk: string) => {
    const badges: any = {
      low: 'bg-[#fdf9ec] text-[#C8952A]',
      medium: 'bg-[#fdf9ec] text-[#C8952A]',
      high: 'bg-[#fdf9ec] text-[#C8952A]'
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Link
              href="/admin"
              className="bg-[#111111] hover:bg-[#111111] text-white px-6 py-3 rounded-lg font-semibold transition-colors whitespace-nowrap text-center"
            >
              Back to Dashboard
            </Link>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg">
                <i className="ri-vip-crown-line text-2xl text-[#C8952A]"></i>
              </div>
            </div>
            <p className="text-sm text-gray-600 mb-1">VIP Customers</p>
            <p className="text-3xl font-bold text-gray-900">{stats.vip}</p>
            <p className="text-sm text-[#C8952A] font-semibold mt-2">Spent &gt; $1,000</p>
          </div>

          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg">
                <i className="ri-refresh-line text-2xl text-[#C8952A]"></i>
              </div>
            </div>
            ...
            <p className="text-sm text-[#C8952A] font-semibold mt-2">More than 1 order</p>
          </div>

          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg">
                <i className="ri-alert-line text-2xl text-[#C8952A]"></i>
              </div>
            </div>
            <p className="text-sm text-gray-600 mb-1">At Risk</p>
            <p className="text-3xl font-bold text-gray-900">{stats.atRisk}</p>
            <p className="text-sm text-[#C8952A] font-semibold mt-2">Inactive &gt; 90 days</p>
          </div>

          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-lg">
                <i className="ri-line-chart-line text-2xl text-[#C8952A]"></i>
              </div>
            </div>
              ...
              <div key={customer.id} className="bg-white rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start space-x-4">
                    <div className="w-16 h-16 flex items-center justify-center bg-gradient-to-br from-emerald-500 to-[#0d0d0d] rounded-full text-white text-2xl font-bold">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <p className="text-2xl font-bold text-[#C8952A]">{formatCurrency(customer.lifetimeValue)}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="flex-1 bg-gray-200 rounded-full h-2">
                        <div
                          className={`h-2 rounded-full ${customer.engagementScore >= 80 ? 'bg-[#111111]' :
                            customer.engagementScore >= 60 ? 'bg-[#111111]' :
                              customer.engagementScore >= 40 ? 'bg-[#111111]' : 'bg-[#111111]'
                            }`}
