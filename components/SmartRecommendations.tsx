            ...
            >
              <div className="relative aspect-square overflow-hidden bg-gray-100">
                ...
                {product.originalPrice && (
                  <div className="absolute top-3 left-3 bg-[#111111] text-white px-3 py-1 rounded-full text-xs font-bold">
                    -{Math.round(((product.originalPrice - product.price) / product.originalPrice) * 100)}%
                  </div>
                )}
              </div>
              <div className="p-4">
                <h3 className="font-semibold text-gray-900 mb-2 line-clamp-2 group-hover:text-[#C8952A] transition-colors">
                  {product.name}
                </h3>
                    ...
                    {[...Array(5)].map((_, i) => (
                      <i
                        ...
                        className={`ri-star-${i < Math.floor(product.rating) ? 'fill' : 'line'} text-sm ${
                          i < Math.floor(product.rating) ? 'text-[#C8952A]' : 'text-gray-300'
                        }`}
                      ></i>
                    ))}
                  </div>
