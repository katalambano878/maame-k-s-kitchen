...
export default function MobileFilterDrawer({ isOpen, onClose }: MobileFilterDrawerProps) {
  ...
  const categories = ['Banku & Soup', 'Ghanaian Dishes', 'Ghanaian Dishes', 'Fufu & Light Soup', 'Sides', 'Drinks'];
    ...
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[90vh] flex flex-col">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-4 rounded-t-2xl">
          ...
          {getActiveFiltersCount() > 0 && (
            <div className="flex items-center justify-between mt-3">
              ...
              <button
                onClick={clearAll}
                className="text-sm text-[#C8952A] font-medium whitespace-nowrap"
              >
                Clear All
              </button>
            </div>
              ...
              {categories.map((category) => (
                <label key={category} className="flex items-center space-x-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 text-[#C8952A] border-gray-300 rounded focus:ring-[#C8952A]"
                  />
                ...
                <button
                  ...
                  className={`px-4 py-2 text-sm rounded-lg border transition-colors whitespace-nowrap ${
                    selectedSizes.includes(size)
                      ? 'bg-[#111111] text-white border-[#C8952A]'
                      : 'bg-white text-gray-700 border-gray-300 hover:border-[#C8952A]'
                  }`}
                >
                  {size}
                </button>
              ...
              {[4, 3, 2, 1].map((rating) => (
                <label key={rating} className="flex items-center space-x-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 text-[#C8952A] border-gray-300 focus:ring-[#C8952A]"
                  />
                  <div className="flex items-center space-x-1">
                    {[...Array(5)].map((_, i) => (
                      <div key={i} className="w-4 h-4 flex items-center justify-center">
                        <i className={`${i < rating ? 'ri-star-fill text-[#C8952A]' : 'ri-star-line text-gray-300'} text-sm`}></i>
                      </div>
                    ))}
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4">
          <button
            onClick={onClose}
            className="w-full bg-[#111111] text-white py-3 rounded-lg font-medium hover:bg-[#111111] transition-colors whitespace-nowrap"
          >
            Show Results ({234})
          </button>
        </div>
