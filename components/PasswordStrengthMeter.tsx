...
export default function PasswordStrengthMeter({ password }: PasswordStrengthMeterProps) {
  ...
  const getStrengthColor = () => {
    if (strength < 40) return 'bg-[#C8952A]';
    if (strength < 70) return 'bg-[#C8952A]';
    if (strength < 90) return 'bg-[#C8952A]';
    return 'bg-[#fdf9ec]0';
  };
    ...
    <div className="mt-2 space-y-2">
      <div className="flex items-center gap-2">
        ...
        <span className={`text-xs font-medium ${
          strength < 40 ? 'text-[#C8952A]' :
          strength < 70 ? 'text-[#C8952A]' :
          strength < 90 ? 'text-[#C8952A]' :
          'text-[#C8952A]'
        }`}>
          {getStrengthText()}
        </span>
        ...
        <div className="space-y-1">
          {feedback.map((item, index) => (
            <div key={index} className="flex items-center gap-1.5 text-xs text-gray-600">
              <i className="ri-close-circle-line text-[#C8952A] w-3 h-3 flex items-center justify-center"></i>
              <span>{item}</span>
            </div>
          ))}
        </div>
      ...
      {strength >= 90 && (
        <div className="flex items-center gap-1.5 text-xs text-[#C8952A]">
