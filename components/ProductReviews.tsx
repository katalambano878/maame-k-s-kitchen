    ...
    <div className="bg-white rounded-xl shadow-sm p-8">
      ...
      {reviews.length === 0 && !showReviewForm ? (
        <div className="text-center py-8 mb-8 border-b border-gray-200">
          ...
          <button
            ...
            className="bg-[#111111] hover:bg-[#111111] text-white px-6 py-2 rounded-lg font-semibold transition-colors"
          >
            Write a Review
          </button>
        </div>
        ...
        <>
            ...
            <div className="text-center">
              ...
              <div className="flex items-center justify-center mb-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <i
                    key={star}
                    className={`ri-star-${star <= Math.round(averageRating) ? 'fill' : 'line'} text-xl ${star <= Math.round(averageRating) ? 'text-[#C8952A]' : 'text-gray-300'
                      }`}
                  ></i>
                ))}
              </div>
                ...
                {ratingDistribution.map((dist) => (
                  <div key={dist.star} className="flex items-center space-x-3">
                    <button
                      ...
                      className="flex items-center space-x-1 hover:text-[#C8952A] transition-colors"
                    >
                      <span className="text-sm font-medium w-6">{dist.star}</span>
                      <i className="ri-star-fill text-[#C8952A] text-sm"></i>
                    </button>
                    <div className="flex-1 h-3 bg-gray-200 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[#C8952A] transition-all duration-300"
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            {!showReviewForm && (
              <button
                onClick={() => setShowReviewForm(true)}
                className="bg-[#111111] hover:bg-[#111111] text-white px-6 py-2 rounded-lg font-semibold transition-colors whitespace-nowrap"
              >
                Write a Review
              </button>
            )}
      ...
      {showReviewForm && (
        <form onSubmit={handleSubmitReview} className="bg-gray-50 rounded-xl p-6 mb-8">
          ...
          <div className="mb-4">
            ...
            <div className="flex space-x-2">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <i
                    className={`ri-star-${star <= reviewForm.rating ? 'fill' : 'line'} text-3xl ${star <= reviewForm.rating ? 'text-[#C8952A]' : 'text-gray-300'
                      }`}
                  ></i>
                </button>
              ))}
          ...
          {!user && (
            <div className="mb-4 p-4 bg-[#fdf9ec] text-[#a07020] rounded-lg">
          ...
          <div className="flex space-x-3">
            <button
              ...
              className="bg-[#111111] hover:bg-[#111111] text-white px-6 py-3 rounded-lg font-semibold transition-colors whitespace-nowrap disabled:opacity-50"
            >
          ...
          <div key={review.id} className="pb-6 border-b border-gray-200 last:border-0">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center space-x-3">
                <div className="w-12 h-12 flex items-center justify-center bg-[#fdf9ec] rounded-full text-[#C8952A] font-bold text-lg">
                ...
                <div>
                  <div className="flex items-center space-x-2">
                    ...
                    {review.verified && (
                      <span className="flex items-center text-xs text-[#C8952A] bg-[#fdf9ec] px-2 py-1 rounded">
              ...
              <div className="flex items-center space-x-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <i
                    key={star}
                    className={`ri-star-${star <= review.rating ? 'fill' : 'line'} text-lg ${star <= review.rating ? 'text-[#C8952A]' : 'text-gray-300'
                      }`}
                  ></i>
                ))}
              </div>
            ...
            <div className="flex items-center space-x-4 text-sm">
              <button className="flex items-center space-x-1 text-gray-600 hover:text-[#C8952A] transition-colors">
